...
import 'CancelledApiServiceModel.dart';
class CancelledApiService{

Future<CancelledApiServiceModel>cancelledOreders(CancelledApiServicerequest req, BuildContext context)async
{
  DataConnectionStatus status = await internetChecker();
  if (status == DataConnectionStatus.connected) {


    final String url = "https://teleoappapi.com/api/orders/vendor/cancelled";
    ...
    if(responce.statusCode==200||responce.statusCode == 400)
    {
      ...
      return cancelledApiServiceModel;
    }
    else{
      final body =  responce.body;
        final error = cancelledApiServiceModelFromJson(body);
        print(error);
    }
...
class CancelledApiServicerequest{
  String vtoken = DataStorage.getVendorToken(),id = DataStorage.getVendorId();
  CancelledApiServicerequest({this.vtoken,this.id});
