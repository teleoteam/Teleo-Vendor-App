...
class UpfordeliveryitemsApiService{
Future<UpforDeliveryitemsModel> getUpForDeliveryItem(UpfordeliveryitemsApiServicerequest req ,BuildContext context)async{
  DataConnectionStatus status = await internetChecker();
  if (status == DataConnectionStatus.connected) {
  final String url = "https://teleoappapi.com/api/orders/vendor/getorders/items/upfordelivery/${req.orderId}";
...
if (request.statusCode == 200) {
        ...
        print(upforDeliveryitemsModel);
        return upforDeliveryitemsModel;
      } else {
        final body = request.body;
        final error = upforDeliveryitemsModelFromJson(body);
        print(error);
      }
}else {
      showDialog(
...
class UpfordeliveryitemsApiServicerequest{ 
  String vtoken = DataStorage.getVendorToken(),id = DataStorage.getVendorId(),orderId;
  UpfordeliveryitemsApiServicerequest({this.orderId,this.vtoken,this.id});
